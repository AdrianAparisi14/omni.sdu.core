...
class URRobotSim(XFormPrim):
    ...
    def init_motion_control(self):
        ...
        world_pose = self.get_world_pose() # self._controller_handle.self.get_world_pose()
    ...
    def get_interpolated_traj_cartesian_space(
        self, current_pose, target_pose, velocity, seed_q=None
    ):
        print("target_pose input:", target_pose)
        target_pose_lula = lula.Pose3(
            lula.Rotation3(R.from_rotvec(target_pose[3:6]).as_matrix()),
            target_pose[0:3],
        )
